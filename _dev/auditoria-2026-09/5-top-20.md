...
> —no semanal: con siete, una ejecución que se retrase ya deja pasar el plazo—, a la raíz de la
> API REST, sin tocar tablas y con la clave publicable leída del propio repositorio. Falla a
> propósito si la nube no contesta bien. ⚠️ Y queda dicho lo que no cubre: GitHub apaga los
> `schedule` de un repositorio con **60 días sin empujones**; hoy se publica a diario, y el día
> que eso cambie la respuesta es el plan de pago.
>
...
> opciones del juego.
>
> **Y queda dicho lo que NO se hizo.** La **sopa de letras sigue sin teclado**: se juega
> arrastrando con eventos de puntero sobre la cuadrícula, no con `click` por celda, así que
> necesita su propio diseño. Y el **control de tamaño de letra de la aplicación del maestro**
> queda para su propia pasada, con dos caminos ya descartados **medidos**: el `+25 %` en cascada
> que usan las misiones **rompe la maquetación por 764 px** (`app.css` tiene 594 tamaños en px y
> 30 en rem, así que la cascada no reparte), y `zoom` sobre el contenedor no desborda pero
> **desalinea las coordenadas del puntero**, lo que rompería el arrastre de la barra de grupos.
>
> Lo comprueba **`_dev/verifica-teclado.js`**, que clasifica una ficha sin tocar la pantalla.
