...
(function () {
  ...
  window.MISION_EN = {
    ...
    data: {
      ...
      qzData: [
        { q: 'What is the cycle a robot repeats while it is switched on?', o: ['a) Read the sensors → decide → move the actuators → repeat', 'b) Sleep → dream → wake up', 'c) Act → switch off → charge up', 'd) Decide → forget → repeat'], c: 0 },
        { q: 'What is a robot’s program?', o: ['a) A TV channel', 'b) The robot’s battery', 'c) The name its owner gives it', 'd) The list of exact instructions it runs step by step'], c: 3 },
        ...
        { q: 'The robot has to move forward 6 equal squares. Which block is best to use?', o: ['a) A variable', 'b) A line sensor', 'c) A loop: repeat 6 times FORWARD', 'd) The WAIT instruction'], c: 2 },
        { q: 'What is a variable for in a robot’s program?', o: ['a) For storing a number, such as how many times it turned', 'b) For painting the robot', 'c) For charging the battery', 'd) For switching the sensors off'], c: 0 },
        { q: 'What is pseudocode?', o: ['a) A broken-down robot', 'b) The program written in plain language before loading it into the robot', 'c) A secret language machines speak', 'd) A kind of sensor'], c: 1 },
        { q: 'The robot crashed into the wall. What does that mean?', o: ['a) That the robot got angry', 'b) That the wall moved', 'c) That the robot is useless now', 'd) That the program has a mistake and needs debugging'], c: 3 },
