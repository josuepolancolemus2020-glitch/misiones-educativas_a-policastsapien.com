...
function genProblemaItems() {
  return _shuffleF(_opProblemCtx, _opRnd).slice(0, 3).map(ctx => {
...
function genEvalOp() {
  ...
  s1.innerHTML = '<div class="eval-section-title">I. División: decimal entre entero <span class="eval-pts">20 pts · 4 pts c/u</span></div><p style="font-size:0.82rem;color:var(--gray);margin-bottom:0.5rem;">Nivel básico. Resuelve el proceso en tu cuaderno y escribe la respuesta en la casilla.</p>';
  ...
  s2.innerHTML = '<div class="eval-section-title">II. Transforma el divisor decimal <span class="eval-pts">10 pts · 2 pts c/u</span></div><p style="font-size:0.82rem;color:var(--gray);margin-bottom:0.5rem;">Escribe el cociente. (Pista: multiplica dividendo y divisor por 10.)</p>';
  ...
  s3.innerHTML = '<div class="eval-section-title">III. Encuentra el número que falta <span class="eval-pts">20 pts · 4 pts c/u</span></div><p style="font-size:0.82rem;color:var(--gray);margin-bottom:0.5rem;">Nivel intermedio. Escribe el número que hace verdadera cada división. Piensa al revés: ¿qué número falta para que funcione?</p>';
  ...
  s4.innerHTML = '<div class="eval-section-title">IV. Problemas de la vida real <span class="eval-pts">30 pts · 10 pts c/u</span></div><p style="font-size:0.82rem;color:var(--gray);margin-bottom:0.5rem;">Nivel avanzado. Plantea la división en tu cuaderno, resuélvela y escribe la respuesta.</p>';
...
function gradeEvalOp() {
  ...
  d.denItems.forEach((it, i) => { const el = document.querySelector(`[data-den="${i}"]`); const ok = _isOpNumOk(el ? el.value : '', it.ans); if (el) { el.classList.toggle('eval-input-ok', ok); el.classList.toggle('eval-input-no', !ok); } if (ok) { det.den += 4; total += 4; } setEvalFeedback('evalFbDen' + i, ok, ok ? 'Correcto. +4 pts' : 'Revisar. R/ ' + it.ans); });
  d.trItems.forEach((it, i) => { const el = document.querySelector(`[data-trn="${i}"]`); const ok = _isOpNumOk(el ? el.value : '', it.ans); if (el) { el.classList.toggle('eval-input-ok', ok); el.classList.toggle('eval-input-no', !ok); } if (ok) { det.trn += 2; total += 2; } setEvalFeedback('evalFbTrn' + i, ok, ok ? 'Correcto. +2 pts' : 'Revisar. R/ ' + it.ans); });
  d.mssItems.forEach((it, i) => { const el = document.querySelector(`[data-mss="${i}"]`); const ok = _isOpNumOk(el ? el.value : '', it.ans); if (el) { el.classList.toggle('eval-input-ok', ok); el.classList.toggle('eval-input-no', !ok); } if (ok) { det.mss += 4; total += 4; } setEvalFeedback('evalFbMss' + i, ok, ok ? 'Correcto. +4 pts' : 'Revisar. R/ ' + it.ans); });
  ...
  { const el = document.querySelector('[data-rerr="0"]'); const ok = _isOpNumOk(el ? el.value : '', d.retoErr.ans); if (el) { el.classList.toggle('eval-input-ok', ok); el.classList.toggle('eval-input-no', !ok); } if (ok) { det.reto += 5; total += 5; } setEvalFeedback('evalFbRerr', ok, ok ? '¡Error detectado! +5 pts' : 'Revisar. R/ ' + d.retoErr.ans); }
  { const el = document.querySelector('[data-rmag="0"]'); const ok = _isOpNumOk(el ? el.value : '', d.retoMag.ans); if (el) { el.classList.toggle('eval-input-ok', ok); el.classList.toggle('eval-input-no', !ok); } if (ok) { det.reto += 5; total += 5; } setEvalFeedback('evalFbRmag', ok, ok ? '¡Magia del punto dominada! +5 pts' : 'Revisar. R/ ' + d.retoMag.ans); }
  const res = document.getElementById('evalOpAutoResult');
  if (res) { res.className = 'eval-auto-result ' + (total >= 70 ? 'eval-auto-pass' : 'eval-auto-risk'); res.innerHTML = `<strong>Resultado: ${total}/100 pts</strong><br><span>÷ Entero: ${det.den}/20 · Transforma: ${det.trn}/10 · Número que falta: ${det.mss}/20 · Problemas: ${det.prb}/30 · Retos: ${det.reto}/20</span>`; }
...
function printEvalOp() {
  ...
  let s2 = `<div class="sec-title"><span>II. Transforma el divisor decimal</span><div class="obt-row"><span class="obt-lbl">Obtenido:</span><span class="obt-line"></span><span class="obt-pct">de 10 pts</span></div></div><p class="opx-instr">Escribe la forma equivalente con divisor entero (×10) y el cociente. 2 pts c/u.</p><div class="rnd-print-grid">${trTbl(d.trItems.slice(0,trH),0)}${trTbl(d.trItems.slice(trH),trH)}</div>`;
  ...
  let s3 = `<div class="sec-title"><span>III. Encuentra el número que falta</span><div class="obt-row"><span class="obt-lbl">Obtenido:</span><span class="obt-line"></span><span class="obt-pct">de 20 pts</span></div></div><p class="opx-instr">Nivel intermedio. Escribe el número que hace verdadera cada división. Piensa al revés. 4 pts c/u.</p><div class="rnd-print-grid">${mssTbl(d.mssItems.slice(0,mssH),0)}${mssTbl(d.mssItems.slice(mssH),mssH)}</div>`;
  let s4 = `<div class="sec-title"><span>IV. Problemas de la vida real</span><div class="obt-row"><span class="obt-lbl">Obtenido:</span><span class="obt-line"></span><span class="obt-pct">de 30 pts</span></div></div><p class="opx-instr">Nivel avanzado. Plantea la división, resuelve el proceso en el reverso de la hoja y escribe la respuesta con su unidad. 10 pts c/u.</p>`;
