...
const evalTFBank=[
  {q:'Un lugar que está al norte también puede estar al este.',a:true,k:'tf-norte-este'},
  {q:'La longitud de un lugar puede llegar a 250°.',a:false,k:'tf-long-max'},
  {q:'Las líneas que miden la longitud van de un polo al otro.',a:true,k:'tf-polo-a-polo'},
  {q:'Las coordenadas sirven también en el mar, donde no hay calles ni rótulos.',a:true,k:'tf-mar'},
  {q:'Honduras queda al sur de la línea que parte la Tierra en norte y sur.',a:false,k:'tf-honduras-sur'},
  {q:'Cada franja de la hora mide 15° de ancho.',a:true,k:'tf-15'},
...
const evalMCBank=[
  ...
  {q:'¿Cuántos meridianos de un grado caben alrededor de la Tierra?',o:['a) 90','b) 180','c) 360','d) 100'],a:2,k:'mc-360'},
  {q:'Si en Tegucigalpa son las 8:00, ¿qué hora es una franja más al este?',o:['a) 7:00','b) 9:00','c) 8:00','d) 20:00'],a:1,k:'mc-huso-este'},
  {q:'¿Con qué líneas se mide la longitud?',o:['a) los paralelos','b) los meridianos','c) los trópicos','d) los círculos polares'],a:1,k:'mc-meridianos'},
  {q:'¿Qué coordenadas pueden ser de un lugar de Honduras?',o:['a) 14° N, 87° O','b) 14° S, 87° E','c) 40° N, 3° O','d) 34° S, 58° O'],a:0,k:'mc-honduras'},
  {q:'¿Por qué la Tierra se reparte en franjas de hora?',o:['a) porque cada país escoge la suya','b) porque la Tierra gira y el sol no alumbra a todos a la vez','c) porque los relojes fallan','d) porque la Luna tapa al Sol'],a:1,k:'mc-por-que-husos'},
  {q:'¿Por qué en Honduras hace calor casi todo el año?',o:['a) porque está en las montañas','b) porque tiene dos mares','c) porque está cerca de la línea del medio de la Tierra, donde el sol pega casi derecho','d) porque es un país pequeño'],a:2,k:'mc-calor'},
  {q:'¿Qué dato NO sirve para ubicar un punto en el mapa?',o:['a) la latitud','b) la longitud','c) el color del país','d) las dos coordenadas juntas'],a:2,k:'mc-no-sirve'},
  {q:'Un lugar en el meridiano cero y en la línea del medio de la Tierra tiene coordenadas…',o:['a) 90°, 90°','b) 180°, 0°','c) 0°, 180°','d) 0°, 0°'],a:3,k:'mc-cero-cero'},
  {q:'Dos lugares con la misma latitud están…',o:['a) a la misma distancia de la línea del medio','b) en el mismo país','c) siempre a la misma hora','d) en el mismo meridiano'],a:0,k:'mc-misma-latitud'}
];
const evalCPBank=[
  {q:'El sistema que ubica un punto con latitud y longitud se llama coordenadas ___.',a:'geográficas',acc:['geográficas'],k:'cp-geograficas'},
  {q:'La Tierra se reparte en ___ franjas de hora.',a:'24',acc:['24','veinticuatro'],k:'cp-24'},
  {q:'Las líneas que dan la vuelta a la Tierra de este a oeste se llaman ___.',a:'paralelos',acc:['paralelos'],k:'cp-paralelos'},
  {q:'La longitud se cuenta hacia el este o hacia el ___.',a:'oeste',acc:['oeste'],k:'cp-oeste'},
  {q:'La latitud de un lugar al sur de la línea del medio lleva la letra ___.',a:'S',acc:['S','sur'],k:'cp-s'},
  {q:'Los dos extremos de la Tierra, arriba y abajo, se llaman ___.',a:'polos',acc:['polos'],k:'cp-polos'},
  {q:'El punto exacto donde está un lugar en el mapa es su ___.',a:'ubicación',acc:['ubicación'],k:'cp-ubicacion'},
  {q:'La longitud se mide contando desde el meridiano ___.',a:'cero',acc:['cero','0'],k:'cp-cero'},
  {q:'En «14° N», el número dice los ___ de latitud.',a:'grados',acc:['grados'],k:'cp-grados'},
  {q:'Una línea que solo existe en los mapas, y no en el suelo, es una línea ___.',a:'imaginaria',acc:['imaginaria'],k:'cp-imaginaria'}
];
const evalPRBank=[
  {term:'Ecuador',def:'Parte la Tierra en norte y sur',k:'pr-ecuador'},
  ...
  {term:'Antípoda',def:'El punto del otro lado de la Tierra',k:'pr-antipoda'}
];
...
function genEval() {
    ...
    cpItems.forEach((item, i) => {
        ...
        const qHtml = item.q.replace('___', '<input class="eval-cp-input" type="text" data-ecp="' + i + '" autocomplete="off" style="min-width:110px;">');
        d.innerHTML = `<div class="eval-q"><span class="eval-num">${i + 1}</span><span class="eval-q-text">${qHtml}</span></div><div class="eval-answer">${item.a}</div><div class="eval-item-feedback" id="evalFbEcp${i}" aria-live="polite"></div>`;
        s1.appendChild(d);
    });
    out.appendChild(s1);
    ...
    tfItems.forEach((item, i) => {
        const d = document.createElement('div'); d.className = 'eval-item eval-auto-item';
        d.innerHTML = `<div class="eval-q"><span class="eval-num">${i + 6}</span><span class="eval-q-text">${item.q}</span></div><div class="eval-tf-opts"><label class="eval-tf-opt"><input type="radio" name="tf${i}" value="V"> Verdadero</label><label class="eval-tf-opt"><input type="radio" name="tf${i}" value="F"> Falso</label></div><div class="eval-answer">${item.a ? 'Verdadero' : 'Falso'}</div><div class="eval-item-feedback" id="evalFbEtf${i}" aria-live="polite"></div>`;
        s2.appendChild(d);
    });
    out.appendChild(s2);
    ...
    mcItems.forEach((item, i) => {
        ...
        d.innerHTML = `<div class="eval-q"><span class="eval-num">${i + 11}</span><span class="eval-q-text">${item.q}</span></div><div class="eval-mc-opts">${optsHtml}</div><div class="eval-answer">${item.o[item.a]}</div><div class="eval-item-feedback" id="evalFbEmc${i}" aria-live="polite"></div>`;
        s3.appendChild(d);
    });
    out.appendChild(s3);

    // IV. Términos Pareados (16-20)
    const prItems = _pickF(evalPRBank, 5, rng);
    const shuffledDefs = _shuffleF(prItems, rng);
