...
// ---- Helpers ----
let _opRnd = Math.random; /* la Prueba Operativa lo reemplaza por _evalRng(100000+forma) para formas deterministas */
...
function genDivisibleItems() {
  const divisors = _shuffleF([2, 3, 5, 9, 10, 11], _opRnd).slice(0, 5);
  return divisors.map(dv => {
    let num;
    if (_opRnd() < 0.5) { num = dv * _opRint(Math.ceil(100 / dv), Math.floor(9999 / dv)); }
